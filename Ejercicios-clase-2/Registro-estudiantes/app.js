...
const {imprimirEstudiantes} = require('./others.js')

const registroEstudiantes = () => {
    let totalEstudiantes = readlineSync.question("Ingrese la cantidad de estudiantes a registrar: ")
    let estudiantes = []
    for (let i=0; i < totalEstudiantes; i++) {
        const nombre = readlineSync.question("Ingrese nombre del estudiante: ")
        let edad = readlineSync.question(`Ingrese edad de ${nombre}: `)
